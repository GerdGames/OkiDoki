...
UdpProtocol::SendPendingOutput()
{
   ...
   if (_local_connect_status) {
      //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("Last Frame recorded by local connect : %i "), (_local_connect_status->last_frame)));
      memcpy(msg->u.input.peer_connect_status, _local_connect_status, sizeof(UdpMsg::connect_status) * UDP_MSG_MAX_PLAYERS);
   } else {
      GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, ("local connect status is invalid "));
...
UdpProtocol::OnLoopPoll(void *cookie)
{
   ...
   case Syncing:
      ...
      if (_last_send_time && _last_send_time + next_interval < now) {
         ...
         GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Looping Sync");
         SendSyncRequest();
      }
      break;
...
{
   //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Sending Sync Request");
...
UdpProtocol::SendMsg(UdpMsg *msg)
{
   ...
   /*GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("Sending message to address %i "), _peer_addr.sin_addr.S_un.S_addr));
   GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("Sending message to port %i "), _peer_addr.sin_port));*/
...
UdpProtocol::Synchronize()
{
   if (_udp) {
      ...
      GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Start Syncing");
      SendSyncRequest();
   }
}
...
UdpProtocol::OnSyncReply(UdpMsg *msg, int len)
{
   ...
   if (--_state.sync.roundtrips_remaining == 0) {
      ...
      GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Now Running");
      ...
      QueueEvent(evt);
      GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Round Trip Reply Sync");
      SendSyncRequest();
   }
   return true;
}
