...
{
   //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Doing a Poll!");
   if (!_sync.InRollback()) {
      _poll.Pump(0);
      ...
      if (!_synchronizing) {
         ...
         //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Setting local frame number");
         ...
         if (total_min_confirmed >= 0) {
            ...
            //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Setting last confirmed frame");
...
{
   int i;
   //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Polling 2 Players");
   ...
   //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Polled 2 Players");
   //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("Minimum confirmed frame: %i"), total_min_confirmed));
                                ...
                                int size)
{
   ...
   //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Adding Local Input");

   if (_sync.InRollback()) {
      GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "In Rollback");
      return GGPO_ERRORCODE_IN_ROLLBACK;
   }
   if (_synchronizing) {
      //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Not Synced");
   ...
   if (input.frame != GameInput::NullFrame) { // xxx: <- comment why this is the case
      ...
      //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("setting local connect status for local queue to  : %i "), (input.frame)));
      _local_connect_status[queue].last_frame = input.frame;

      // Send the input to all the remote players.
      for (int i = 0; i < _num_players; i++) {
         if (_endpoints[i].IsInitialized()) {
             //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Sending Input");
   ...
   switch (evt.type) {
      case UdpProtocol::Event::Input:
         if (!_local_connect_status[queue].disconnected) {
            ...
            //GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, FString::Printf(TEXT("setting remote connect status for queue to : %i "), (evt.u.input.input.frame)));
...
Peer2PeerBackend::PlayerHandleToQueue(GGPOPlayerHandle player, int *queue)
{
   ...
   if (offset < 0 || offset >= _num_players) {
      GEngine->AddOnScreenDebugMessage(-1, 15.0f, FColor::Red, "Invalid Player Handle");
